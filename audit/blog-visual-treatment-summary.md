...
- **Two priority downgrades:** `find-color-code-from-image` and `how-to-make-a-qr-code` moved P1 → P2 — both are clear and useful, but neither is as load-bearing as an empty heading or a post built entirely around an unillustrated comparison.
...
|---|---|---|
| 1 | 13 | Empty heading waiting for the visual, or the post is built around a comparison/mechanism that's currently unillustrated |
...
### 1. Four posts with an empty heading already waiting for the visual

Unchanged from v1 — still the highest-confidence P1 candidates, and still true after the review: the prose in each of these was written assuming a table or chart sits directly under the named heading.

| Slug | Empty heading | Visual type |
|---|---|---|
...
New from the review: none of these four should ship with the chart as the section's *only* content — each needs a short introductory sentence plus its `accessible_equivalent` (all four are `html-table`) alongside the visual, not instead of prose.
...
1. **`shrink-bmp-files`** — precise-comparison / hybrid+svg / exact / EN+ES / shared / empty-heading. ChatGPT-specified composition, reproducible-benchmark data, SVG build.
2. **`social-media-image-sizes-2026`** — reference-sheet / hybrid+svg+html-table / exact / EN+ES / shared / empty-heading. Volatile real-world data, authoritative sourcing, and the semantic-HTML-table-plus-SVG pattern together.
